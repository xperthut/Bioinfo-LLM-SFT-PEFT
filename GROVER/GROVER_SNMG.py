...
class Inference:
    def __init__(self, gpu_id, model, test_data, tokenizer, optimizer, training_args):
        self.gpu_id = gpu_id
        self.data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer)
        self.training_args = training_args
        ...
        self.trainer = transformers.Trainer(model=model,
                                   ...
                                   compute_metrics=compute_metrics,
                                   train_dataset=None,
                                   eval_dataset=self.test_data.dataset,
                                   data_collator=self.data_collator,
                                   optimizers=(optimizer, None)
                                   )
    ...
    def predict(self):
        # Evaluate
        results = self.trainer.predict(self.test_data.dataset)
        results_path = os.path.join(self.training_args.output_dir, "results", f"{self.training_args.run_name}_Test")
        os.makedirs(results_path, exist_ok=True)
        with open(os.path.join(results_path, "inference_results.json"), "w") as f:
            json.dump(results, f)
    
        if self.gpu_id == 0 and False:
