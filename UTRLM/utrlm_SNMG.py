...
wandb.init(project="utrlm_mpra", entity="Nanobody", mode="offline")
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, 
        ...
        print(f"[GPU{self.global_rank}] Unique labels found: {self.num_labels}")
...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="multimolecule/utrlm-te_el")
...
@dataclass
class DataArguments:
    data_path: str = field(default="../../Data/mpara/", metadata={"help": "Path to the training data."})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    run_name: str = field(default="UTRLM_SNMP", metadata={"help": "Name of the run for logging."})
    optim: str = field(default="adamw_torch")
    model_max_length: int = field(default=15, metadata={"help": "Maximum sequence length."})
    ...
    auto_find_batch_size: bool = field(default=True, metadata={"help": "Automatically find the best batch size."})
    half_precision_backend: str = field(default="auto", metadata={"help": "Backend for half precision."})
...
def calculate_metric_with_sklearn(logits: np.ndarray, labels: np.ndarray):
    ...
    print(f"Valid predictions: {sum(valid_predictions)}, Valid labels: {sum(valid_labels)}")
    return {
...
def load_model(model_args: ModelArguments, training_args: TrainingArguments, num_labels: int=2):
    # load model
    config = UtrLmConfig()
    config.model_max_length = training_args.model_max_length
    config.problem_type = "single_label_classification" #if num_labels == 2 else "multi_label_classification"
    #config.num_labels = 1 if num_labels == 2 else num_labels,  # binary classification
...
def load_trained_model(training_args: TrainingArguments):
    ...
    model = UtrLmForSequencePrediction.from_pretrained(saved_model_path, config=config, cache_dir=training_args.cache_dir, trust_remote_code=True)
    return model
...
def train():
    ...
    if training_args.do_train:
        ...
        print(f'[GPU{local_rank}] Model loaded successfully. \n{model}')
